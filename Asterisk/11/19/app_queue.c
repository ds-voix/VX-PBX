...
/*
 2015: New properties by Dmitry Svyatogorov ds@vo-ix.ru
 Now I publish code as is, not ready to propose it for upstream asterisk.
 Changed places are marked with "PnD!" to facilitate further work.

 poslog: Log each position change of each queue entry (many events!)
 offset: Shift announced position (and calculated time) by this value
 minhold: Minimum on-hold time for further calculations
 announce-backoff: Supress position increasement announces, when "no". Default "yes".
...
#define MAX_PERIODIC_ANNOUNCEMENTS 128          /*!< The maximum periodic announcements we can have */
...
struct queue_ent {
	...
	int periodicannouncelimit;     	       /*!< Periodic announcements replay counter */ /* PnD! */
...
struct call_queue {
	...
	int announcetimelimit;              /*!< Ceil time to be announced */ /* PnD! */
	int announcedelay;                  /*!< Delay seconds before doing first position/time announce */ /* PnD! */
	int announcefrequency;              /*!< How often to announce their position */
	int minannouncefrequency;           /*!< The minimum number of seconds between position announcements (def. 15) */
	int periodicannouncefrequency;      /*!< How often to play periodic announcement */
	int periodicannouncelimit;          /*!< Limit periodic announcements replay */ /* PnD! */
	...
	int holdsecs;                       /*!< REAL head hold time in seconds for calculate announcements */ /* PnD! */
...
static void init_queue(struct call_queue *q)
{
	...
	q->announcetimelimit = 30; /* Default 30 seconds */ /* PnD! */
	q->announcedelay = 0; /* PnD! */
...
// <PnD!>
		...
		q->announcetimelimit = atoi(val);
		if (q->announcetimelimit < 1) {
			q->announcetimelimit = 1;
		}
	} else if (!strcasecmp(param, "announce-delay")) {
		q->announcedelay = atoi(val);
	} else if (!strcasecmp(param, "periodic-announce-limit")) {
		q->periodicannouncelimit = atoi(val);
// </PnD!>
...
// <PnD!>
	...
	} else if (!strcasecmp(param, "minhold")) {
		q->minhold = atoi(val);
		if (q->minhold < 1) {
			q->minhold = 1; /* At least 1s */
		}
// </PnD!>
...
static int say_position(struct queue_ent *qe, int ringing)
{
	...
	if (qe->parent->poslog && (qe->pos != qe->ppos)) {
        ...
        int xpos = qe->pos;
        int opos = qe->opos; // + qe->parent->offset;

	if (!qe->parent->backoff && (qe->last_pos_said > 0) && (qe->pos > qe->last_pos_said)) {
		pos = qe->last_pos_said + qe->parent->offset;
		xpos = qe->last_pos_said;
	}

	if ((qe->pos == 1) && (dt > qe->parent->minhold)) {
		ao2_lock(qe->parent);
...
// <PnD!>
	if ((qe->last_pos_said == xpos) && ((now - qe->last_pos) < qe->parent->announcefrequency)) {
		return 0;
	}
// Don't annoy with "more than"!
	if ((qe->parent->announceposition == ANNOUNCEPOSITION_MORE_THAN)
	   && (pos > qe->parent->announcepositionlimit)
	   && ((now - qe->last_pos) < qe->parent->announcefrequency)){
		return 0;
	}
// </PnD!>
	...
	if (qe->parent->announceposition == ANNOUNCEPOSITION_YES ||
		...
		xpos <= qe->parent->announcepositionlimit)) { /* PnD! */
			announceposition = 1;
	}
	...
	if (announceposition == 1) {
		...
		if (pos == 1) { /* PnD! */
			...
			if (qe->parent->announceposition == ANNOUNCEPOSITION_MORE_THAN && pos > qe->parent->announcepositionlimit){ /* PnD! */
				/* More than Case*/
				...
				res = ast_say_number(qe->chan, pos, AST_DIGIT_ANY, ast_channel_language(qe->chan), NULL); /* Needs gender */ /* PnD! */
				if (res) {
					goto playout;
				}
			}
...
//	avgholdmins = labs(((qe->parent->holdtime + 30) - (now - qe->start)) / 60);
        int hold = qe->parent->holdsecs;
        if (hold < qe->parent->minhold) {
         hold = qe->parent->minhold;
        }
	avgholdmins =  hold * pos / 60;
	if ((qe->last_hold_said > 0) && (avgholdmins > (qe->last_hold_said + (qe->parent->announcefrequency / 60) + 1))) {
		avgholdmins = qe->last_hold_said + (qe->parent->announcefrequency / 60) + 1;
	}
	if (avgholdmins > qe->parent->announcetimelimit) {
		avgholdmins = qe->parent->announcetimelimit;
	}
        qe->last_hold_said = avgholdmins;
// </PnD!>
...
//		avgholdsecs = hold * pos / qe->parent->roundingseconds; //No, must be 0..60
...
posout:
	if (qe->parent->announceposition) {
		ast_verb(3, "Told %s in %s their queue position (which was %d)\n",
			ast_channel_name(qe->chan), qe->parent->name, xpos); /* PnD! */
	}
	if (say_thanks) {
...
static void recalc_holdtime(struct queue_ent *qe, int newholdtime)
{
	...
	qe->parent->holdsecs = newholdtime / qe->opos; /* PnD! */
	ao2_unlock(qe->parent);
}
...
#define DATA_EXPORT_CALL_QUEUE(MEMBER)					\
	...
	MEMBER(call_queue, announcetimelimit, AST_DATA_INTEGER)	        \
	MEMBER(call_queue, announcedelay, AST_DATA_INTEGER)	        \
	...
	MEMBER(call_queue, periodicannouncelimit, AST_DATA_INTEGER)	\
	...
	MEMBER(call_queue, offset, AST_DATA_INTEGER)			\
	MEMBER(call_queue, minhold, AST_DATA_INTEGER)			\
...
#define DATA_EXPORT_QUEUE_ENT(MEMBER)						\
	...
	MEMBER(queue_ent, ppos, AST_DATA_INTEGER)				\
