...
// PnD!  !!!Hardcoded SQL-string max-size!!! -8/~
static int maxsize = 2048, maxsize2 = 2048;
...
#define LENGTHEN_BUF1(size)                                               \
			do {                                                          \
				...
				if (ast_str_strlen(sql) + size + 1 > ast_str_size(sql)) { \
					if (ast_str_make_space(&sql, ((ast_str_size(sql) + size + 3) / 2048 + 1) * 2048) != 0) {	\
...
#define LENGTHEN_BUF2(size)                               \
			do {                                          \
				if (ast_str_strlen(sql2) + size + 1 > ast_str_size(sql2)) {  \
					if (ast_str_make_space(&sql2, ((ast_str_size(sql2) + size + 3) / 2048 + 1) * 2048) != 0) {	\
						ast_log(LOG_ERROR, "Unable to allocate sufficient memory.  Insert CDR failed.\n");	\
						ast_free(sql);                    \
						ast_free(sql2);                   \
						AST_RWLIST_UNLOCK(&psql_columns); \
...
static int pgsql_log(struct ast_cdr *cdr)
{
	...
	if (connected) {
		...
		char buf[1025], escapebuf[2049], *value; /* PnD! */
		int first = 1;
